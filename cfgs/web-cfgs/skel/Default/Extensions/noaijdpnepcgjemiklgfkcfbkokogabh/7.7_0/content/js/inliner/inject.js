...
function inlinerInjectTranslateNextSentence(doc) {
    ...
    while (srcText == "") {
        if (inlinerCoreSelectNextSentence(doc) != 0) {
            if (SL_SEGMENTS_CNT < 0 && document.getElementById('inlinercovertext')) document.getElementById('inlinercovertext').innerHTML=FExtension.element("extInlineLimit").replace("XXX",SL_SEGMENTS);
...
function inlinerInjectCoverShow(doc) {
    ...
    doc.getElementById(coverText.id).innerHTML = FExtension.element("extTranslating")+" ...";
