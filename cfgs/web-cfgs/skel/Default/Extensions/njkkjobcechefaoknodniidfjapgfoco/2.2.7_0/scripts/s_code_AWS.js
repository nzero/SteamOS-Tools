...
s.linkDownloadFileTypes = "exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,psd,dmg"
s.trackExternalLinks = true
s.linkInternalFilters = "javascript:,weatherbug"
...
s.useCommerce = true
s.varUsed = "eVar1"
s.eventList = "event1" //Abandon,Success,Error
...
function s_doPlugins(s) {
	...
	/******
	Commented out the assignments to prop1, prop2, prop8; swapped prop12 and prop13 (one was reporting days, the other hours). 
	These were interfering with the data collected in OmnitureCtl.ascx.

	See PRCHG00031485 for details.

	-Edwin Liu 09-14-2007
	*****/

	s.hier1 = s.getPageName();
	//s.campaign=s.getQueryParam('zcode')
	//s.eVar2=s.getQueryParam('')
	...
	//s.prop8=s.getCustomPagePath(s.campaign,"s_p8","zcode=");

	s.setupFormAnalysis();

	/* Plugin Example: timeparting EST */
	s.prop12 = s.getTimeParting('d', '-5', '2007'); // Set day
	s.prop13 = s.getTimeParting('h', '-5', '2007'); // Set hour 
	s.prop14 = s.getTimeParting('w', '-5', '2007'); // Set Weekend / Weekday
...
+ "if(thismin>30){mint='00'};if(thish>=12){ap='PM';thish=thish-12};"
...
+ "/'+(s.ssl?'':'metrics')+'.weatherbug.com/b/ss/'+s.un+'/1/H.6-pdv-"
